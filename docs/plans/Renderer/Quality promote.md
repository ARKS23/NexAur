...
### 4.7 PR-R38.7：Local Shadow Quality Pass / Cube Shadow Polish
...
### 4.8 PR-R38.8：Rect Soft Shadow / PCSS Baseline
...
```cpp
struct RenderRectSoftShadowSettings {
    bool enabled = false;
    float softness_scale = 1.0f;
    float min_filter_radius = 1.0f;
    float max_filter_radius = 8.0f;
    uint32_t blocker_search_taps = 8;
    uint32_t pcf_taps = 16;
};
```
