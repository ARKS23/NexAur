...
### 4.5 PR-R38.5：LTC Rect Light Specular
...
- LTC LUT 作为 renderer 静态采样资源，可以挂在 frame-global descriptor set 后续 binding，或建立独立 lighting resource descriptor set。
- LUT sampler 使用 `linear + clamp`；LUT texture 必须按 linear float 读取，不走 sRGB。
- LUT 加载失败时 shader 或 C++ 应 fallback 到 R38 representative point specular，避免资源缺失导致黑屏或 NaN。
...
### 4.6 PR-R39：Color Grading + Anti-Aliasing Polish
...
### 4.7 PR-R40：Reflection Probe / SSR v1
...
### 4.8 PR-R41：Material / Texture Sampling Polish
...
### 4.9 PR-R42：Atmosphere / Fog / Depth Polish
...
5. 增加 contact shadow 或 SSAO，让 block 落地。
6. 锁定 exposure / bloom 默认值，避免靠过曝制造亮度。
7. 保留 Cornell debug view：direct、indirect、AO、shadow、final lit。
