...
### 4.14 PR-R40.8：Reflection Probe Capture Scheduling / Baked Asset

目标：

- 在 R40.7 的手动 capture 之上，补齐多 probe 更新策略、资源预算和可持久化 baked asset 流程。

主要工作：

- 新增 probe capture scheduler：dirty queue、每帧 capture budget、priority、update interval、camera / scene dirty trigger。
- 支持多 probe capture 资源缓存，限制最大 resident baked probe 数量和 cubemap resolution。
- Editor 增加多 probe 操作入口：
  - 顶部菜单或 Scene 工具栏提供 `Bake Reflection Probes` / `Bake All Probes`。
  - Properties Panel 保留单个 probe 的 `Bake Probe`，并增加 preview、dirty / fresh 状态、Clear Baked Data。
  - 后续 Project / Asset 面板显示 generated baked cubemap asset 和引用状态。
- 可选接入 AssetManager：将 baked cubemap 导出为 generated environment asset，场景 reload 后可复用。
- Renderer Debug 显示 capture queue、pending count、last capture time、memory footprint。
...
- `Bake All Probes` 能从菜单 / Scene 工具栏触发，并复用 capture scheduler 的预算策略。
- 单 probe `Bake Probe` 与全局 `Bake All Probes` 的状态反馈一致：pending、capturing、fresh、failed。
- viewport resize / scene reload 后 baked resource 生命周期稳定。
- 保存为 baked asset 时，SceneSerializer 能恢复对应 asset handle。
