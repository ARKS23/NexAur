...
assets/shaders/Renderer/Vulkan/
  ...
  post/
    ...
    vulkan_smaa_edge.hlsl
    vulkan_smaa_blend.hlsl
    vulkan_smaa_neighborhood.hlsl
  reflection/
    vulkan_ssr.hlsl
  shadow/
...
- 如果项目节奏希望把它升格为 `PR-R39`，则原 `PR-R39.0：Color Grading / Post Tone Polish` 与 `PR-R39.5：SMAA Anti-Aliasing Polish` 顺延到后续编号；从依赖关系看，LTC 应当排在 R38 之后、Color Grading 之前或之后都可独立推进。
...
### 4.9 PR-R39.0：Color Grading / Post Tone Polish （已完成）

目标：

- 在已有 HDR / ACES / Bloom 基础上增加最终画面调色收口。
- 先稳定黑位、白位、中间调、饱和度和最终输出观感，为后续 SMAA 提供干净输入。

主要工作：

- Color grading：exposure offset、contrast、saturation、temperature / tint、vignette、white point / black point。
- Sharpen：可选，默认低强度或关闭，用于后续 AA 后恢复轻微细节损失。
- LUT 预留：保留 3D LUT / 2D strip LUT 的设置入口和 shader 接口，但第一版不引入复杂 LUT 资产管理。
- Debug view：pre-tonemap HDR、post-tonemap LDR、color graded output、bloom contribution。
- Render Settings / Renderer Debug：暴露 color grading enable、参数值、debug output 和 fallback 状态。

后续升级：

- SMAA：单独进入 R39.5，避免在 color grading PR 中同时引入多 pass AA 资源和额外中间 target。
- TAA：需要 motion vector、jitter、history buffer。
- 自动曝光：需要 luminance histogram 或 mip reduction，建议后置，不要和第一版 grading 混在一起。

建议代码组织：

- `RenderColorGradingSettings` 或扩展 `RenderPostProcessSettings`：保存手动调色参数。
- `assets/shaders/Renderer/Vulkan/post/vulkan_color_grading.hlsl` 或 `common/color_grading.hlsli`：封装 exposure / tone map 后的调色函数。
- `VulkanPostProcessPass`：继续负责 HDR -> LDR 输出；第一版不新增额外 target。
- Render Settings 面板将 color grading 放在 Post Processing 区，Renderer Debug 显示当前调色参数。

范围约束：

- R39.0 只做手动 color grading / sharpen / debug view。
- 不做 SMAA / TAA；抗锯齿留给 R39.5。
- 不做自动曝光。
...
- 关闭 color grading 后可回到当前 ACES / Bloom baseline。
- Bloom 不再靠过曝撑画面，强度有健康默认值。
- Debug 构建通过，HLSL 编译通过；必要 smoke 覆盖 Render Settings 参数传递和 Sandbox 短启动。

### 4.9.5 PR-R39.5：SMAA Anti-Aliasing Polish （已完成）

目标：

- 在 R39.0 已完成最终色彩输出校准后，增加不依赖 history buffer 的高质量屏幕空间抗锯齿。
- 优先解决几何边缘、明暗高反差边缘和 Cornell / Sandbox 中明显 staircase 问题。
- 保持 UI / ImGui 不被 SMAA 处理，AA 只作用于 scene color / post-process 图像。

主要工作：

- 新增 `RenderAntiAliasingSettings` 或扩展 post-process settings：`mode = None / SMAA`、edge threshold、max search steps、blend strength。
- 新增 SMAA 三阶段 baseline：
  1. Edge detection：基于 luma 或 color edge 检测边缘。
  2. Blend weight：根据边缘方向和搜索距离估计混合权重。
  3. Neighborhood blend：对最终 scene color 做邻域混合。
- 新增 SMAA intermediate target：edge target、blend target；target lifetime 随 viewport / swapchain resize 正确重建。
- Render Settings 面板提供 AA mode 和 SMAA 参数；Renderer Debug 显示 SMAA enabled、edge target、blend target、当前 debug view。
- Debug view 可查看 SMAA edge mask、blend weight 和 final AA output。

建议代码组织：

- `VulkanSmaaTarget`：管理 edge / blend 中间图。
- `VulkanSmaaPass`：封装 edge detection、blend weight、neighborhood blend 三个 sub-pass 或 graph pass。
- `assets/shaders/Renderer/Vulkan/post/vulkan_smaa_edge.hlsl`
- `assets/shaders/Renderer/Vulkan/post/vulkan_smaa_blend.hlsl`
- `assets/shaders/Renderer/Vulkan/post/vulkan_smaa_neighborhood.hlsl`
- `VulkanRendererSystem` 只负责把 SMAA pass 接入 graph，不把 SMAA 算法细节塞进主 renderer。

范围约束：

- 不做 TAA、DLAA、FSR / TSR 或 temporal accumulation。
- 不做自动 exposure、color grading 或 Bloom 调参；这些属于 R39.0。
- 第一版可以使用 shader 内置 search/area 近似，外部 SMAA area/search texture 可作为后续 refinement。
- 不处理透明 UI；UI 应在 SMAA 后绘制或保持独立路径。

验收标准：

- SMAA 开启后几何边缘锯齿明显减少，画面不过度糊。
- SMAA 关闭后可回到 R39.0 输出。
- viewport resize 后 edge / blend target 正确重建。
- Debug view 能查看 edge mask 和 blend weight。
- Debug 构建通过，HLSL 编译通过；必要 smoke 覆盖 AA settings 传递和 Sandbox 短启动。

### 4.10 PR-R40：Reflection Probe v1 （已完成）

目标：

- 解决当前 IBL 只提供全局反射，室内或局部场景反射不匹配的问题。
- 让金属和低 roughness 材质可以在局部空间内反射手动指定的 cubemap，而不是永远反射全局天空盒。
- 建立 Reflection Probe 的 scene -> render frame -> Vulkan draw list -> shader 数据链路，为后续 SSR fallback 和 runtime capture 留接口。

建议路线：

- 先做手动 Reflection Probe，不做 runtime cubemap capture。
- 支持一个主 probe 或最近 probe，避免第一版进入复杂 probe blending。
- Forward shader 中增加 local specular reflection 混合策略，只替换/混合 IBL specular，不影响 diffuse IBL。
- 支持 box projection，让室内 probe 的反射方向更贴近局部空间。
- 增加 probe influence / probe specular / probe bounds debug，保证效果可解释。

建议数据组织：

- `ReflectionProbeComponent`
  - `AssetHandle environment_asset`：手动绑定 cubemap / HDR environment 资产。
  - `glm::vec3 box_extents`：probe AABB 半尺寸，用于影响范围和 box projection。
  - `float intensity`：局部反射强度。
  - `float blend_distance`：靠近边界时的淡出距离。
  - `bool enabled`：是否参与渲染。
  - `bool box_projection`：是否启用 box projected cubemap lookup。
- `RenderFrameReflectionProbe`
  - 存储 world center、box extents、intensity、blend distance、asset handle 和 projection flag。
  - 进入 `RenderSceneFrame`，由 `RenderSceneFrameBuilder` 从 scene component 收集。
- `VulkanDrawList`
  - 增加 active probe 描述和对应 `VulkanEnvironmentResource*`。
  - v1 只选择一个 probe：优先 camera 所在 probe，其次最近 probe。
- `VulkanFrameLightingResource`
  - 增加 probe 参数 uniform：center、extents、intensity、blend distance、enabled、box projection。
  - probe cubemap descriptor 可以复用 environment resource 的 prefiltered cubemap 和 BRDF LUT。
- `SceneSerializer` / `PropertiesPanel`
  - 保存和编辑 `ReflectionProbeComponent`。
  - Properties Panel 第一版通过 Environment Path 导入 HDR / environment asset，并提供 Scene Env / Clear 操作；完整资产拖拽可后置到编辑器资产系统。
  - Editor debug draw 显示 probe bounds。
...
2. 计算 probe influence：

   ```hlsl
   float3 local_p = abs(world_position - probe_center);
   float3 distance_to_edge = probe_extents - local_p;
   float inside = all(distance_to_edge >= 0.0f) ? 1.0f : 0.0f;
   float edge_fade = saturate(min(distance_to_edge.x, min(distance_to_edge.y, distance_to_edge.z)) / blend_distance);
   float probe_weight = inside * edge_fade * probe_intensity;
   ```
...
4. 如果启用 box projection，从当前 world position 沿 reflection ray 与 probe AABB 求交，用 `hit_position - probe_center` 作为 cubemap lookup direction。

5. 用 roughness 选择 prefiltered mip，用 BRDF LUT 做 specular IBL 修正。

6. 混合时只替换 specular：

   ```hlsl
   ibl.specular = lerp(ibl.specular, probe_specular, probe_weight);
   ```

代码组织建议：

- `source/Engine/Function/Scene/component.h`：新增 `ReflectionProbeComponent`。
- `source/Engine/Function/Scene/scene_serializer.cpp`：保存/加载 probe component。
- `source/Engine/Function/Renderer/data/render_scene_frame.h`：新增 `RenderFrameReflectionProbe`。
- `source/Engine/Function/Renderer/frontend/render_scene_frame_builder.cpp`：收集 probe。
- `source/Engine/Function/Renderer/Vulkan/frontend/vulkan_draw_list.h/.cpp`：解析 active probe 和 GPU resource。
- `source/Engine/Function/Renderer/Vulkan/resources/vulkan_frame_lighting_resource.*`：上传 probe 参数。
- `assets/shaders/Renderer/Vulkan/common/pbr_reflection_probe.hlsli`：封装 influence、box projection 和 specular sample。
- `assets/shaders/Renderer/Vulkan/vulkan_forward.hlsl`：在 IBL specular 后接入 probe specular。
- `source/Engine/Editor/Panels/properties_panel.cpp`：编辑 probe 参数。
- `source/Engine/Editor/Panels/render_settings_panel.cpp`：增加 reflection debug view 入口。
...
- 不做 parallax-corrected 多 probe blending、probe priority stack 或自动 capture scheduling。
- 不要求所有材质都明显变化；验证重点放在 metallic / low roughness 测试面。

验收标准：

- 室内/局部场景的金属和光滑材质不再只反射全局天空盒。
- Probe influence debug view 能显示影响范围和边界淡出。
- Probe specular debug view 能单独显示局部反射贡献。
- Editor 可以通过 Scene Hierarchy 创建 Reflection Probe entity，或在 Properties 的 Add Component 菜单给选中实体添加 `ReflectionProbeComponent`。
- Probe entity 可以用现有 Transform gizmo 手动摆放，Properties Panel 可编辑 environment path / asset、box extents、intensity、blend distance、enabled 和 box projection。
- Viewport / debug draw 能显示 probe bounds，方便确认手动放置位置和影响范围。
- SceneSerializer 能保存/加载 Reflection Probe component，重新打开场景后 probe 位置、资产和参数保持一致。
- Probe 关闭时可稳定回退到当前全局 IBL。
- Debug 构建通过；必要 smoke 覆盖 scene serializer、render frame builder 和 render settings/debug enum 传递。

### 4.11 PR-R40.5：SSR Foundation （已完成）

目标：

- 为后续 SSR v1 建立最小屏幕空间反射数据链路和 debug 视图，不追求最终画质。
- SSR 作为 Reflection Probe 的屏幕空间补充，命中失败时必须 fallback 到 probe / IBL。

主要工作：

- 新增 SSR settings：enabled、max distance、max steps、thickness、stride、roughness fade、edge fade、intensity。
- 新增 SSR intermediate target：reflection color、hit mask / confidence。
- 评估 forward renderer 需要的 screen-space inputs：
  - scene color：使用 post 前 HDR scene color。
  - depth：复用当前 scene depth。
  - normal / roughness：优先新增一个轻量 material meta target；如果暂缓，则 SSR Foundation 只输出 debug hit mask，不进入最终 composite。
- 新增 `VulkanSsrTarget` / `VulkanSsrPass`，放在 reflection 或 post 前后处理链路中。
- 新增 debug view：SSR Hit Mask、SSR Ray Steps、SSR Raw Reflection。
- 已落地为 foundation debug path：SSR pass 读 HDR scene color + scene depth，输出 raw reflection 和 hit mask 两个中间 target；由于当前 forward renderer 尚未输出 normal / roughness target，R40.5 不混入 Final Lit。

范围约束：

- 不做 temporal accumulation。
- 不做 stochastic SSR / blue-noise resolve。
- 不做透明物体 SSR。
- 如果 normal / roughness target 未完成，SSR 不进入 Final Lit，只提供 debug foundation。

验收标准：

- SSR pass 能稳定创建、resize 和释放 target。
- SSR settings 能从 RenderSettings 传递到 Vulkan pass。
- Debug view 能看到 hit mask / miss 区域。
- SSR 关闭或 miss 时不会破坏 probe / IBL fallback。
- Debug 构建通过；必要 smoke 覆盖 settings 传递和 Sandbox 短启动。

### 4.12 PR-R40.6：SSR v1 Polish （已完成）

目标：

- 在 R40.5 foundation 上完成可用于画面展示的 SSR v1。

核心算法：

1. 从 depth 重建 view-space position。
2. 使用 normal 和 view direction 计算 reflection ray。
3. 在 view-space ray march，并逐步投影回屏幕 UV。
4. 用 depth thickness 判断 hit。
5. 命中后做少量 binary refinement。
6. 采样 scene color 得到 reflected color。
7. 根据 roughness、ray distance、屏幕边缘、hit confidence 淡出。
8. miss 或 confidence 低时 fallback 到 Reflection Probe / IBL。

范围约束：

- 不做 temporal denoise / history resolve。
- 不承诺离屏物体反射；离屏内容由 probe fallback 承担。
- 不做多层 depth 或透明 SSR。
- 当前 forward renderer 尚未输出 material normal / roughness meta target，v1 使用 depth-derived normal 和全局 roughness fade 控制；材质级 roughness mask 留到后续 meta target / deferred-lite PR。

验收标准：

- 地面或低 roughness 平面能看到稳定的屏幕空间反射。
- 屏幕边缘和 miss 区域能平滑 fallback 到 probe / IBL。
- 全局 roughness fade 增加时 SSR contribution 合理衰减；材质级 roughness 衰减待 material meta target。
...
- 让 `ReflectionProbeComponent` 不再只能依赖静态 environment asset，而是具备编辑器手动 capture / bake 入口和运行时 probe resource 绑定链路。
- 先交付单 probe、手动 capture、低频更新的稳定资源 / 状态链路，不追求运行时每帧刷新，也不在本 PR 做完整六面场景重绘。

主要工作：

- 扩展 `ReflectionProbeComponent`：resolution、near / far clip、include skybox、dirty flag；SceneSerializer 和 render frame 提取保持这些参数。
- 新增 renderer service capture API：`requestReflectionProbeCapture` / `getReflectionProbeCaptureState`，编辑器可查询 pending / capturing / ready / failed。
- 新增 runtime probe resource 链路：Vulkan 后端按 probe entity id 维护 runtime `VulkanEnvironmentResource`，capture 后 active probe 优先绑定 runtime descriptor。
- runtime resource 支持按 capture resolution 生成 cubemap / prefilter mip / irradiance / BRDF LUT；没有 environment asset 时 fallback 到当前场景环境色或黑色。
- capture/bake 在 R40.7 中等价为一次手动 runtime resource 更新；Bake 不生成持久化 asset。
- Editor 单 probe 操作放在选中 `ReflectionProbeComponent` 后的 Properties Panel / Inspector：
  - `Capture Probe`：请求下一帧生成运行时 cubemap resource。
  - `Bake Probe`：R40.7 中先等价于 capture + 绑定运行时 cubemap，不生成持久化 baked asset。
  - 组件区暴露 resolution、near / far clip、include skybox、dirty flag、last capture status 和错误信息。
- Scene Hierarchy 仍负责创建 / 选中 Reflection Probe；Viewport gizmo 和 probe bounds debug 用于确认 capture 位置和影响范围。
- Renderer Debug 显示 active runtime baked resource、capture resolution、last capture status。

范围约束：

- 第一版只做手动 bake，不做 every-frame runtime capture。
- 第一版只保证一个 active baked probe 路径稳定，不做复杂多 probe 更新调度。
- 不做 probe asset 持久化；baked cubemap 可以先作为运行时资源存在。
- 不在 R40.7 增加全局 `Bake All`；全场景入口留给 R40.8。
- 不在 R40.7 做从 probe center 重绘六个 cubemap face 的完整 GPU capture；当前 baseline 先从 probe/scene environment asset 或 fallback color 生成 runtime probe IBL resource。
- 不做 diffuse GI；capture 结果先服务 specular reflection。

验收标准：

- 选中 Reflection Probe entity 后，Properties Panel 的 `Capture Probe` / `Bake Probe` 可以生成 runtime cubemap resource，并让该 probe 参与 specular IBL。
- 未选中 probe 或 probe disabled 时，不显示或禁用单 probe capture / bake 操作，并给出明确状态。
- capture / scene reload 不保留已不存在 probe 的 runtime image view、sampler 或 descriptor。
- capture 时不会递归采样自身 reflection probe；完整 GPU face capture 留到后续时也必须保持该约束。
- Debug 构建通过；必要 smoke 覆盖 settings / component 保存。

### 4.14 PR-R40.8：Reflection Probe Capture Scheduling / Baked Asset （已完成）

目标：

- 在 R40.7 的手动 capture 之上，补齐多 probe 更新策略、资源预算和运行期 baked asset identity 流程。

主要工作：

- 新增 probe capture scheduler：按 probe entity 去重的 pending queue、每帧 capture budget、priority 排序和 dirty-only 入口。
- 支持多 probe runtime capture 资源缓存，限制最大 resident baked probe 数量和 cubemap resolution；超预算时按 LRU / generation 淘汰非 active probe。
- Editor 增加多 probe 操作入口：
  - `Scene` 菜单提供 `Bake Reflection Probes` / `Bake Dirty Reflection Probes`。
  - Properties Panel 保留单个 probe 的 `Bake Probe`，并显示 dirty / fresh 状态、runtime baked asset handle 和 `Clear Baked Data`。
  - 后续 Project / Asset 面板显示 generated baked cubemap asset、缩略图和引用状态。
- 接入 AssetManager：runtime bake 成功时登记 runtime-generated `EnvironmentMap` asset identity；`ReflectionProbeComponent` 保存 `baked_environment_asset`，SceneSerializer 可写回 / 恢复该引用。
- Renderer Debug 显示 capture queue、pending count、budget、resident count / limit 和 last captured probe。

范围约束：

- 不做 async compute / background streaming；先使用 graphics queue 上的显式预算更新。
- 不做 probe blending priority stack；active probe 选择仍保持 R40 简单策略，或只做轻量排序。
- 不做跨场景全局 bake database。
- 不做从 probe center 重绘六个 cubemap face 的完整 GPU scene capture；真实位置捕获留给 R40.9。
- 不做真正 cubemap / HDR 文件导出；当前 baked asset 是运行期 identity + runtime GPU resource，持久化文件导出留给后续 generated asset pipeline。

验收标准：

- 多个 probe 可以被手动或 dirty queue 依次 bake，不阻塞整帧过久。
- `Bake Reflection Probes` / `Bake Dirty Reflection Probes` 能从 Scene 菜单触发，并复用 capture scheduler 的预算策略。
- 单 probe `Bake Probe` 与全局 bake 状态反馈一致：pending、capturing、fresh、failed。
- viewport resize / scene reload 后 baked resource 生命周期稳定。
- 保存为 baked asset identity 时，SceneSerializer 能恢复对应 runtime-generated asset handle；缺少磁盘路径的 runtime handle 不承诺跨进程复用。
- Debug 构建通过；必要 smoke 覆盖序列化和 scheduler settings 传递。

### 4.15 PR-R40.9：Reflection Probe True Scene Capture （已完成）

目标：

- 补齐 Reflection Probe 的真实场景捕获链路，让 capture 结果来自 probe 所在世界坐标，而不是继续复用 probe / scene environment asset 或 fallback color。
- 复用 R40.7 / R40.8 已完成的 capture API、scheduler、runtime resource 和 baked asset identity，把“可请求 / 可调度 / 可观测”升级为“真正渲染六面 cubemap”。
- 保持低频、可控、可调试的 editor bake 模式，不引入每帧动态 probe 或复杂 probe blending。

主要工作：

- 新增 reflection probe scene capture target：
  - HDR color cubemap 或 2D array target，6 个 face / layer。
  - capture depth target，随 capture resolution 重建。
  - image usage / layout transition 明确支持 color attachment、sampled 和后续 filtering 输入。
- 为单个 probe 构造六个 capture view：
  - 以 probe world position 为原点。
  - 使用 `+X / -X / +Y / -Y / +Z / -Z` 六个方向和稳定 up vector。
  - 90 度 FOV，near / far 使用 `ReflectionProbeComponent` 的 capture 参数。
  - capture view 不继承 editor camera jitter、viewport UI、gizmo、picking 或 post-process 状态。
- 新增 `ReflectionProbeSceneCapture` graph pass 或等价 backend pass：
  - 对六个 face 渲染 opaque scene geometry 和必要 direct lighting / shadow。
  - `capture_include_skybox == true` 时在 capture 背景中绘制 skybox / scene environment。
  - `capture_include_skybox == false` 时背景保持黑色或明确的空环境，但仍必须捕获局部几何体。
  - capture 时禁止采样当前 probe 的 runtime resource，避免递归反射；可回退到全局 IBL 或禁用 probe specular。
  - 不渲染 ImGui、editor gizmo、object id picking、bloom、SMAA、SSR 或最终 post-process。
- 将 captured cubemap 转换为 probe IBL resource：
  - 为 `VulkanEnvironmentResource` 增加从 GPU captured cubemap 构建 / 接管的路径，或新增 dedicated probe environment resource。
  - 基于 captured cubemap 生成 irradiance、prefilter mip 和 BRDF LUT 绑定，避免 CPU readback 成为第一版依赖。
  - capture 失败时保留上一版 ready resource，并把失败原因写入 capture state。
- 接入 R40.8 scheduler / baked identity：
  - 单 probe `Capture Probe`、`Bake Probe` 和 Scene 菜单 `Bake Reflection Probes` 都走真实 scene capture。
  - scheduler 预算按 probe 数和 face 数计量，避免一次性 capture 多个高分辨率 probe 卡住整帧。
  - runtime baked asset identity 继续可序列化；持久化 HDR / cubemap 文件导出仍留给 generated asset pipeline。
- Debug / Editor：
  - Renderer Debug 显示 capture source 为 `SceneCapture`、当前 face、resolution、耗时 / frame budget、失败原因。
  - Effects Debug 或 Probe Debug 能查看 captured cubemap face / layer，确认结果不是天空盒替身。
  - Properties Panel 对当前 baseline 做文案收口：R40.9 后 `Capture Probe` 才表示真实场景捕获。

范围约束：

- 不做 every-frame runtime reflection update；第一版仍是 editor 手动 / dirty queue 低频 capture。
- 不做 async compute、background streaming 或跨线程资源上传。
- 不做透明物体、粒子、UI、后期效果或 SSR 的 probe capture。
- 不做复杂多 probe blending priority stack；active probe 选择沿用 R40 简单策略。
- 不在本 PR 导出持久化 HDR / DDS / KTX cubemap 文件；只保证当前进程内 runtime resource 和 serialized baked identity 行为稳定。
- 不做 diffuse GI；captured cubemap 第一版仍先服务 specular reflection，local diffuse IBL 顺延到 R40.10。

验收标准：

- 在 Cornell / 室内测试场景中，把 Reflection Probe 放到不同位置后，captured reflection 能反映附近墙体、block 和局部灯光差异，而不是始终等于天空盒。
- `Capture Skybox` 打开时，六面 capture 的背景包含 skybox；关闭时天空盒不进入 cubemap，但局部几何体仍被捕获。
- Probe debug face view 能看到六个 face 的场景内容，且不包含 editor UI、gizmo、object id debug 或 post-process 叠加。
- capture 过程中不会递归采样自身 runtime probe，不产生 NaN、黑屏、descriptor dangling 或上一帧 resource 被提前释放。
...
- 在 Reflection Probe true scene capture 稳定后，提供一个明确标注为 baseline 的局部 diffuse IBL 混合路径。
- 这一步只解决“局部环境漫反射颜色不匹配”，不宣称完整 GI。

主要工作：

- 在 runtime / baked probe resource 中生成 irradiance cubemap 或 SH irradiance。
- 扩展 frame globals / shader：在 probe influence 内对 diffuse IBL 做局部 irradiance 混合。
- 新增 `RenderIblDebugMode::ReflectionProbeDiffuse` 或等价 debug view，单独观察 diffuse probe contribution。
- 增加 diffuse intensity、diffuse enabled、leak warning 等 probe 参数或 render settings。
- AO / normal bias 与 probe influence 结合，降低最明显的墙体漏光。

范围约束：

- 不做真正多次 bounce GI。
- 不做 DDGI probe volume、visibility moments、ray traced irradiance 或 lightmap bake。
- 不让 diffuse probe 替代 direct lighting；rect / point / directional light 仍负责直接光。
- 默认值必须保守，避免室内被 diffuse IBL 再次填白。

验收标准：

- 启用 local diffuse IBL 后，室内低频漫反射颜色能跟随 baked probe，而不是只来自全局天空盒。
- 关闭 diffuse probe 后回到 R40.9 的 specular-only scene-captured probe 行为。
...
### 4.18 PR-R42：Atmosphere / Fog / Depth Polish
...
```text
R39.0 Color Grading / Post Tone Polish
  -> R39.5 SMAA Anti-Aliasing Polish
  ...
  -> R40.10 Local Diffuse IBL Baseline
...
Reflection、LTC Area Light、SSR 和 TAA 会进一步提高上限，但不建议在没有光照基线、probe fallback 和 debug view 的情况下提前堆叠。
...
- 如果纳入 R39.0，color grading 应能在关闭时回到当前 ACES / Bloom baseline。
- 如果纳入 R39.5，SMAA 应能在关闭时回到 R39.0 输出，且 UI 不被 AA 污染。
- 如果纳入 R40.5 / R40.6，SSR miss、屏幕边缘和关闭状态必须稳定回退到 Reflection Probe / IBL。
- 如果纳入 R40.7 / R40.8，Reflection Probe runtime capture 必须可手动 bake、可观测、受预算限制，并且不引入递归反射。
- 如果纳入 R40.9，Reflection Probe capture 必须从 probe 所在点真实渲染六面 cubemap，不能继续只复用天空盒 / environment asset。
- 如果纳入 R40.10，Local Diffuse IBL 必须明确是局部 irradiance baseline，不宣称完整 GI，并能关闭回到 specular-only probe。
...
- TAA。
- Auto exposure。
- SSR temporal resolve / stochastic SSR refinement。
- High-frequency runtime reflection capture / async probe streaming。
- Full irradiance probe volume / DDGI。
- Volumetric lighting。
...
### 2026-07-02：PR-R39.0 Color Grading / Post Tone Polish

完成内容：
- `RenderPostProcessSettings` 新增手动 color grading 参数：enable、grade exposure offset、contrast、saturation、temperature / tint、black / white point、vignette 和 sharpen，默认值保持当前 ACES / Bloom baseline。
- `vulkan_post_process.hlsl` 将最终输出拆成 post-tonemap 与 color-graded 两个明确阶段，新增 `Post Tone Map` 和 `Color Graded` debug view，便于确认 tone mapping、grading 和 sharpen 的边界。
- 新增 `common/color_grading.hlsli`，集中封装 white balance、contrast、saturation、black / white point 与 vignette helper，避免主 post shader 膨胀。
- `VulkanPostProcessPass` 扩展 push constants 并在 CPU 侧做有限范围 sanitize / clamp，防止 UI 调参把 NaN 或过宽范围传进 shader。
- Render Settings 面板暴露 color grading / sharpen 控件；Renderer Debug 显示 tone mapping、exposure、bloom 和当前 grading 参数。
- `RenderSettingsSmoke` 覆盖新增 post-process 参数和 `PostToneMap` / `ColorGraded` debug view 的 settings 传递。

范围说明：
- 本 PR 不做 SMAA / TAA / auto exposure；抗锯齿拆到 R39.5，自动曝光保留在后置能力池。
- 第一版不引入 3D LUT / strip LUT 资产管理；当前 shader 组织为后续 LUT 入口预留了清晰边界。
- Color grading 关闭时保留现有 ACES / Bloom 输出路径，sharpen 也随 grading 关闭而关闭。
...
### 2026-07-02：PR-R39.5 SMAA Anti-Aliasing Polish

完成内容：
- 新增 `RenderAntiAliasingSettings` 与 `RenderAntiAliasingMode`，支持 `None / SMAA`、edge threshold、contrast factor、search steps、blend strength。
- 新增 `VulkanSmaaTarget`，管理 post-process 后的 LDR source、edge mask、blend weight 三张中间图，并随 viewport resize 维护 layout。
- 新增 `VulkanSmaaPass`，封装 Edge Detection、Blend Weight、Neighborhood Blend 三阶段 pipeline；第一版不依赖外部 SMAA area/search LUT。
- 新增 `post/vulkan_smaa_edge.hlsl`、`post/vulkan_smaa_blend.hlsl`、`post/vulkan_smaa_neighborhood.hlsl`，接入 CMake HLSL 编译与 shader library。
- Render graph 在 `FinalLit + SMAA` 或 SMAA debug view 时，将 PostProcess 输出转入 `SMAASource`，随后由 SMAA 写回 viewport / swapchain；ImGui 仍在 AA 之后合成，UI 不被 SMAA 处理。
- Render Settings 面板暴露 AA mode 和 SMAA 参数；Renderer Debug 显示 SMAA target、格式、模式和当前参数。
- Effects Debug 新增 `SMAA Edge Mask`、`SMAA Blend Weight`、`SMAA Output`；`RenderSettingsSmoke` 覆盖 AA 参数和 SMAA debug view 传递。

范围说明：
- 本 PR 是 SMAA-style baseline，不引入外部 area/search texture；后续可替换为更接近参考实现的 LUT 版本。
- 不做 TAA、DLAA、FSR / TSR、history buffer 或 temporal accumulation。
- `Post Tone Map` / `Color Graded` 继续保持 R39.0 的 pre-AA debug 边界；需要观察 AA 后结果时使用 `SMAA Output`。
...
### 2026-07-02：PR-R40 Reflection Probe v1

完成内容：
- 新增 `ReflectionProbeComponent`，支持 environment asset、box extents、intensity、blend distance、enabled 和 box projection，并接入 SceneSerializer。
- Scene extraction 输出 `RendererReflectionProbeData`，debug draw 显示 probe AABB 和中心 gizmo；`RenderSceneFrameBuilder` 增加 `RenderFrameReflectionProbe`，默认最多保留 16 个启用 probe。
- `VulkanDrawList` 保留 probe 列表并选择一个 active probe：相机在盒内优先，否则选择离 probe AABB 最近者；probe 未绑定 environment 时回退到场景 Environment / fallback Environment。
- `VulkanForwardPass` 增加 set 3 probe environment descriptor，`VulkanFrameLightingResource` 上传 active probe 参数。
- 新增 `common/pbr_reflection_probe.hlsli`，Forward shader 只混合 / 替换 specular IBL，支持 box-projected cubemap lookup 和边界 blend。
- IBL Debug 新增 `Reflection Probe Influence` 和 `Reflection Probe Specular`；Renderer Debug 显示 reflection probe count、active probe 状态和 probe resource。
- Editor 支持 Scene Hierarchy 创建 Reflection Probe、Add Component 添加 Reflection Probe、Inspector 编辑参数，并通过 Environment Path 导入 probe environment。
- `SceneSerializerSmoke` 覆盖 Reflection Probe 保存 / 加载；`RenderSettingsSmoke` 覆盖 render frame builder 对 probe 的提取和 disabled probe 过滤。

范围说明：
- 本 PR 只做手动 Reflection Probe v1，不做 runtime cubemap capture。
- 本 PR 只绑定一个 active probe，不做多 probe blending、priority stack 或 capture scheduling。
- Probe 只影响 specular IBL，不承担 diffuse GI；SSR 拆到 R40.5 / R40.6。
- 当前编辑器环境资产绑定是路径导入 baseline，完整 Project Panel 拖拽 / asset picker 可在编辑器资产系统 PR 中完善。
...
### 2026-07-02：PR-R40.5 SSR Foundation

完成内容：
- 新增 `RenderSsrSettings`，Render Settings 暴露 enabled、max distance、max steps、thickness、stride、roughness fade、edge fade 和 intensity。
- 新增 `VulkanSsrTarget`，管理 raw reflection 和 hit mask / confidence 两张中间 target，随 viewport resize 重建。
- 新增 `VulkanSsrPass` 和 `reflection/vulkan_ssr_trace.hlsl`，读取 HDR scene color + scene depth，输出 SSR raw reflection 与 hit mask foundation。
- Post Process descriptor 增加 SSR debug inputs；Effects Debug 新增 `SSR Hit Mask`、`SSR Ray Steps`、`SSR Raw Reflection`。
- Renderer Debug 显示 SSR target ready、格式、尺寸和当前 SSR 参数；`RenderSettingsSmoke` 覆盖 SSR settings 传递。

范围说明：
- 当前 forward renderer 尚无 normal / roughness material meta target，R40.5 不把 SSR 混入 Final Lit。
- 当前 SSR trace 使用 depth-derived view normal 和屏幕空间近似步进，只作为后续 R40.6 的资源 / debug / settings foundation。
- SSR 关闭或 miss 时不影响 Reflection Probe / IBL fallback。
...
### 2026-07-02：PR-R40.6 SSR v1 Polish

完成内容：
- SSR trace 从屏幕方向近似步进升级为 view-space ray march：使用 depth 重建 view position / depth-derived normal，通过 projection scale 投影回 UV。
- 命中检测加入 thickness window、屏幕边缘衰减、距离衰减、facing 衰减，并在命中后做 4 次 binary refinement。
- Raw reflection target 保留未预乘的 HDR scene color，hit mask target 只表达几何 confidence，避免 intensity 在 trace 与 composite 阶段重复作用。
- Post Process 在 Final Lit / Post Tone Map / Color Graded 路径中合成 SSR，SSR disabled / miss / confidence 低时自然保留原 forward lighting，也就是继续回退到 Reflection Probe / IBL。
...
### 2026-07-02：PR-R40.7 Reflection Probe Runtime Capture Baseline

完成内容：
- `ReflectionProbeComponent` 新增 runtime capture 参数：resolution、near / far clip、include skybox、dirty flag，并接入 SceneSerializer、Scene extraction 和 RenderSceneFrame。
- Properties Panel 在 Reflection Probe 组件区提供 `Capture Probe` / `Bake Probe`，通过 renderer service 发起单 probe 手动 capture，并显示 pending / capturing / ready / failed 状态。
- Vulkan 后端新增 runtime capture 状态表：按 probe entity id 维护 pending request、runtime `VulkanEnvironmentResource` 和 capture state；scene reload / probe 缺失时释放对应 runtime resource。
- `VulkanEnvironmentResource` 支持按 build settings 生成 cubemap、irradiance、prefilter mip 和 BRDF LUT；runtime capture 可按组件 resolution 控制资源尺寸。
- Active reflection probe 优先绑定 ready 的 runtime environment descriptor；Renderer Debug 显示 Probe Runtime、capture size 和 capture status。

范围说明：
- R40.7 是 runtime resource / editor operation baseline，不做从 probe center 重绘六个 cubemap face 的完整 GPU scene capture。
- 当前 capture source 优先使用 probe 绑定的 environment asset，其次使用 scene environment asset，缺失时 fallback 到环境色；关闭 `Capture Skybox` 时使用黑色 runtime environment。
- `Bake Probe` 在本 PR 中等价于一次 runtime capture，不生成持久化 baked asset；Bake All、scheduler、generated asset 留到 R40.8。
- Probe 仍只影响 specular IBL，不承担 diffuse GI。
...
### 2026-07-02：PR-R40.8 Reflection Probe Capture Scheduling / Baked Asset

完成内容：
- Renderer service capture API 扩展为多 probe queue：请求按 entity 去重、按 `capture_priority` 排序，并以每帧固定 budget 处理。
- Vulkan 后端维护多个 resident runtime reflection probe resource，记录 generation / last-used frame，并在超过 resident limit 时淘汰非 active、非 pending 的旧 resource。
- `ReflectionProbeComponent` 新增 `capture_priority` 和 `baked_environment_asset`，并接入 SceneSerializer、Scene extraction、RenderSceneFrame 和 RenderSettings smoke。
- `Bake Probe` 成功时登记 runtime-generated `EnvironmentMap` baked asset identity；EditorLayer 将 bake state 同步回组件，场景保存时可记录 baked asset reference。
- Scene 菜单新增 `Bake Reflection Probes` / `Bake Dirty Reflection Probes`，统一复用 renderer scheduler；Properties Panel 增加 `Fresh` 状态、baked asset 显示和 `Clear Baked Data`。
- Renderer Debug 显示 capture pending、budget、resident count / limit 和 last captured probe。

范围说明：
- 当前 baked asset 是运行期 asset identity + runtime GPU resource，不导出 HDR / cubemap 文件；跨进程复用和 Project 面板 generated asset 管线留给后续。
- 当前 capture source 仍沿用 R40.7：probe environment asset、scene environment asset 或 fallback color，不做从 probe center 重绘六个 cubemap face 的完整 GPU scene capture；真实位置捕获留给 R40.9。
- Probe 仍只影响 specular IBL，不承担 diffuse GI。
...
### 2026-07-02：PR-R40.9 Reflection Probe True Scene Capture

完成内容：
- 新增 `VulkanReflectionProbeCaptureTarget`，提供 6 层 HDR color cubemap、per-face render target、capture depth 和 readback buffer。
- Reflection Probe capture 不再从 probe / scene environment asset 直接生成 runtime resource，而是从 probe 世界坐标构造六个 90 度 capture view 并渲染 scene cubemap。
- Capture pass 复用现有 skybox、shadow 和 forward pass；`Capture Skybox` 只控制背景是否绘制天空盒，关闭时仍捕获局部几何体。
- capture 时清空 active reflection probe，避免递归采样自身 runtime capture；同时强制关闭 IBL / effects debug 输出，避免 debug color 被烘进 cubemap。
- `VulkanEnvironmentResource` 新增从 captured cubemap pixels 生成 environment / irradiance / prefilter / BRDF LUT 的 runtime 路径。
- R40.8 scheduler、单 probe capture / bake、Bake All / Bake Dirty 和 runtime baked asset identity 继续复用原链路。

范围说明：
- 第一版 true scene capture 使用同步 face capture + readback + CPU IBL 过滤，优先保证结果正确和生命周期清楚；GPU convolution / async capture 留给后续性能 PR。
- 当前 capture 仍只渲染 opaque scene geometry、skybox、direct lighting 和 shadow，不捕获 UI、gizmo、object id、SSR、Bloom、SMAA 或最终 post-process。
- 持久化 HDR / DDS / KTX cubemap 导出仍未实现；baked asset 仍是运行期 identity + runtime GPU resource。
- Probe 仍只影响 specular IBL，不承担 diffuse GI；local diffuse IBL 顺延到 R40.10。
