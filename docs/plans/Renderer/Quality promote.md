...
### 4.6 PR-R38.6：Rect Light Shadow / Area Shadow Baseline
...
- 新增 `RectLightComponent::cast_shadow`、`shadow_range`、`shadow_strength`，继续保持组件语义简单。
- Frame builder 按 `RenderRectShadowSettings::max_shadowed_lights` 给 rect light 分配 shadow slot。
- Vulkan 新增 `VulkanRectShadowTarget`，使用 2D depth array，每个 shadowed rect light 占 1 个 layer。
...
RectLightComponent(cast_shadow)
  ...
  -> VulkanRectShadowTarget(depth array)
...
- `VulkanRectShadowTarget`：管理 depth array、sampler、resize / resolution rebuild。
...
### 4.7 PR-R39：Color Grading + Anti-Aliasing Polish
...
### 4.8 PR-R40：Reflection Probe / SSR v1
...
### 4.9 PR-R41：Material / Texture Sampling Polish
...
### 4.10 PR-R42：Atmosphere / Fog / Depth Polish
...
6. 增加 contact shadow 或 SSAO，让 block 落地。
7. 锁定 exposure / bloom 默认值，避免靠过曝制造亮度。
8. 保留 Cornell debug view：direct、indirect、AO、shadow、final lit。
