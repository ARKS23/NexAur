...
### 4.15 PR-R40.9：Local Diffuse IBL Baseline

目标：

- 在 Reflection Probe capture 稳定后，提供一个明确标注为 baseline 的局部 diffuse IBL 混合路径。
...
- 关闭 diffuse probe 后回到 R40 / R40.8 的 specular-only probe 行为。
- Debug view 能区分 global diffuse IBL 与 local probe diffuse contribution。
- 文档明确记录漏光和非完整 GI 限制。

### 4.16 PR-R41：Material / Texture Sampling Polish
...
### 4.17 PR-R42：Atmosphere / Fog / Depth Polish
...
R39.0 Color Grading / Post Tone Polish
  ...
  -> R40.9 Local Diffuse IBL Baseline
...
- 如果纳入 R40.9，Local Diffuse IBL 必须明确是局部 irradiance baseline，不宣称完整 GI，并能关闭回到 specular-only probe。
...
- 当前 capture source 仍沿用 R40.7：probe environment asset、scene environment asset 或 fallback color，不做从 probe center 重绘六个 cubemap face 的完整 GPU scene capture。
- Probe 仍只影响 specular IBL，不承担 diffuse GI。
