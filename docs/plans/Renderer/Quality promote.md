...
### 4.13 PR-R40.7：Reflection Probe Runtime Capture Baseline

目标：

- 让 `ReflectionProbeComponent` 不再只能手动绑定外部 environment map，而是可以在编辑器中从 probe 位置手动 bake 当前场景。
- 先交付单 probe、手动 capture、低频更新的稳定资源链路，不追求运行时每帧刷新。

主要工作：

- 扩展 `ReflectionProbeComponent`：capture mode、resolution、near / far clip、include skybox、dirty flag、last capture status。
- 新增 `VulkanReflectionProbeCaptureTarget`：HDR cubemap color、per-face depth、mip chain 和 descriptor 生命周期。
- 新增 capture flow：从 probe center 渲染六个 cubemap face，capture 时禁用 SSR / reflection probe contribution，避免递归反射。
- 新增 runtime probe resource：可以从 GPU cubemap 生成 / 更新 probe environment descriptor，并复用 BRDF LUT。
- 生成 specular prefilter mip；irradiance 生成可以保留接口，第一版不强制接入 diffuse lighting。
- Editor Properties 增加 `Bake Probe`、resolution 和 capture status；Renderer Debug 显示 capture target / active baked resource。
...
- 点击 `Bake Probe` 后，probe 使用当前场景生成的 cubemap 参与 specular IBL。
- capture / resize / scene reload 不泄漏 image view、sampler、descriptor 或 depth target。
- capture 时不会递归采样自身 reflection probe。
- Debug 构建通过；必要 smoke 覆盖 settings / component 保存，Sandbox 短启动通过。
...
- Editor 增加 probe preview、dirty / fresh 状态、Bake All、Clear Baked Data。
