...
中间通过 adapter 连接
  ArkVulkanRendererSystem
  ArkRenderResourceCache
  ArkRenderDataTranslator
```
...
更理想的边界是：这些 ARKRenderer 头文件只出现在 `ArkVulkanRendererSystem`、`ArkRenderResourceCache` 和 `ArkRenderDataTranslator` 的实现中。NexAur Editor、Scene、Resource、Gameplay 仍然只看 `RendererService` 和 `RenderDataPacket`。
...
RendererModule
  ...
  ArkVulkanRendererSystem
    owns ark::Renderer
    owns ArkRenderResourceCache
    owns ArkRenderDataTranslator
```
...
Scene / EditorCamera / GameCamera
  ...
  -> ArkVulkanRendererSystem::render()
      -> ArkRenderDataTranslator builds ark::RenderView
      -> ArkRenderResourceCache resolves AssetHandle
      -> ArkRenderDataTranslator builds ark::RenderScene
...
AssetHandle
  ...
  -> ArkRenderResourceCache owns renderer GPU resources
...
```text
source/Engine/Function/Renderer/Backend/ArkVulkan/
  ark_vulkan_renderer_system.h
  ark_vulkan_renderer_system.cpp
  ark_render_resource_cache.h
  ark_render_resource_cache.cpp
  ark_render_data_translator.h
  ark_render_data_translator.cpp
```

### 6.1 ArkVulkanRendererSystem
...
- 管理 viewport 尺寸。
- 持有 `ArkRenderResourceCache`。
- 调用 `ArkRenderDataTranslator`。
- 提供 viewport output / picking 的 Vulkan 过渡实现。

### 6.2 ArkRenderResourceCache
...
转换集中在这里，避免 `ArkVulkanRendererSystem::render()` 膨胀。
...
ArkVulkan -> GLFW_CLIENT_API = GLFW_NO_API
```
...
- NexAur CMake 能发现或构建 `ARKRenderer::ark_renderer`。
...
- OpenGL legacy 仍能启动。
- ArkVulkan path 能拿到 GLFW native window 并创建 surface。

### PR-R5：ArkVulkanRendererSystem 骨架
...
- 实现 `ArkRenderDataTranslator::buildRenderView()`。
...
### PR-R7：ArkRenderResourceCache 第一版
...
- 默认 renderer backend 为 ArkVulkan。
...
3. 顶层 vcpkg / CMake 方案。
4. Window no-api 支持。
5. ArkVulkanRendererSystem 骨架。

中期优先：

1. RenderView 转换。
2. ArkRenderResourceCache。
3. RenderScene 转换。
...
不要继续扩展旧 OpenGL 渲染模块
  ...
  -> 中间用 ArkVulkanRendererSystem adapter 连接
  -> Vulkan 稳定后退役 OpenGL
```
