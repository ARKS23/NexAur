...
Backend internal
  OpenGL old classes
  ARKRenderer Renderer / RenderScene / RenderView / resources
```
...
- 配置系统选择 backend。
- 后续删除 OpenGL 后仍保留 `ArkVulkan` 和 `Unknown` 即可。
...
## 10. ArkVulkanRendererSystem 接口边界

`ArkVulkanRendererSystem` 应实现 `RendererService`，内部依赖 ARKRenderer。

内部成员可以是：

```cpp
class ArkVulkanRendererSystem final : public RendererService {
public:
...
private:
    std::unique_ptr<ArkRenderResourceCache> m_resource_cache;
    std::unique_ptr<ArkRenderDataTranslator> m_translator;
    ark::Scope<ark::Renderer> m_renderer;
    ark::RenderScene m_scene;
    ark::RenderView m_view;
};
```

注意：

- 这里可以 include ARKRenderer public facade。
- 不要把 `ark::Renderer` 暴露到 `RendererService`。
- 不要让 Editor panel include `ark::RenderView`。
...
这会让后续废除 OpenGL、只保留 ARKRenderer Vulkan 路径更自然。
