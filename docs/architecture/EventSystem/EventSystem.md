...
### 3. 数据流向
```
WindowSystem callback
  -> WindowService event callback
  -> Engine::onEvent
  -> ModuleManager::dispatchEvent
  -> UI / Editor / Renderer / Runtime modules
```

## 用法
- engine.cpp
```C++
 getRequiredModuleService<WindowService>()
    ->setEventCallback(NX_BIND_EVENT_FN(Engine::onEvent));
