...
namespace NexAur {
    ...
    bool VulkanObjectIdPass::createPipeline() {
        if (!m_pipeline_cache) {
            NX_CORE_ERROR("VulkanObjectIdPass requires a pipeline cache.");
            return false;
        }
        ...
        desc.debug_name = "ObjectId";
        desc.shader_program = VulkanShaderProgramId::ObjectId;
        desc.color_format = m_object_id_format;
        desc.depth_format = m_depth_format;
        desc.push_constant_ranges = { push_constant_range };
        desc.color_write_mask = VK_COLOR_COMPONENT_R_BIT;
