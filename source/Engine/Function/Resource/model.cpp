...
namespace NexAur {
    ...
    void Model::processNode(aiNode* node, const aiScene* scene, const glm::mat4& parent_transform) {
        const glm::mat4 node_transform = parent_transform * toGlmMatrix(node->mTransformation);

        // 处理当前节点的所有网格体
        for (unsigned int i = 0; i < node->mNumMeshes; ++i) {
            aiMesh* mesh = scene->mMeshes[node->mMeshes[i]];
            m_meshes.push_back(processMesh(mesh, scene, node_transform));
        }

        // 递归处理所有子节点
        for (unsigned int i = 0; i < node->mNumChildren; ++i) {
            processNode(node->mChildren[i], scene, node_transform);
        }
    }

    Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene, const glm::mat4& node_transform) {
    ...
    std::string Model::loadMaterialTexturePath(aiMaterial* material, int type) {
        ...
        if (material->GetTextureCount(aiType) > 0) {
            ...
            return normalizePathString(std::filesystem::path(m_directory) / texture_path);
        }
